...
K <- 100 
Covariate <- c("x_current"
               , "age"
               , "bmi"
               ) 
vari.index <- which(colnames(inla_df) %in% c("posttiter", Covariate))
...
for (k in 1:K) {
  ...
  H1N1.test_data$posttiter      <- 
  H3N2.test_data$posttiter      <-
  Victoria.test_data$posttiter  <- NULL

  H1N1.test_data <- as.h2o(H1N1.test_data)
  H3N2.test_data <- as.h2o(H3N2.test_data)
  Victoria.test_data <- as.h2o(Victoria.test_data)
  # h2o.test.ped.rf   <- as.matrix(h2o.predict(rf_model, test_data))[, 1]
  
  x <- which(colnames(train_data) %nin% "posttiter") 
  ...
  dl_model   <- h2o.deeplearning( x              = x,
                                  ...
                                  seed           = 1234)
  # h2o.test.ped.dl   <- round(as.matrix(h2o.predict(dl_model, test_data))[, 1])
  
  test_preds.1 <- as.vector(predict(dl_model, H1N1.test_data))
  test_preds.2 <- as.vector(predict(dl_model, H3N2.test_data))
  test_preds.3 <- as.vector(predict(dl_model, Victoria.test_data))
